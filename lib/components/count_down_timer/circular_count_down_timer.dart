...
import 'package:burgan_ui/components/count_down_timer/circular_count_down_timer_painter.dart';
import 'package:burgan_ui/components/count_down_timer/count_down_timer_text_format.dart';
...
class CircularCountDownTimerState extends State<CircularCountDownTimer> with TickerProviderStateMixin {
  ...
  String get time {
    ...
    if (widget.isReverse && !widget.autoStart && !countDownController!.isStarted) {
      if (widget.timeFormatterFunction != null) {
        return Function.apply(widget.timeFormatterFunction!, [_getTime, Duration(seconds: widget.duration)]).toString();
      } else {
  ...
  String _getTime(Duration duration) {
    // For HH:mm:ss format
    if (widget.textFormat == CountdownTimerTextFormat.HH_MM_SS) {
      return '${duration.inHours.toString().padLeft(2, '0')}:${(duration.inMinutes % 60).toString().padLeft(2, '0')}:${(duration.inSeconds % 60).toString().padLeft(2, '0')}';
    }
    // For mm:ss format
    else if (widget.textFormat == CountdownTimerTextFormat.MM_SS) {
      return '${(duration.inMinutes % 60).toString().padLeft(2, '0')}:${(duration.inSeconds % 60).toString().padLeft(2, '0')}';
    }
    // For ss format
    else if (widget.textFormat == CountdownTimerTextFormat.SS) {
